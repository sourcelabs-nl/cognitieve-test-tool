  ...
  return (
    <section className="screen">
      <header className="screen-header">
        ...
        <button className="link-button" onClick={onSwitchProfile}>profiel wisselen</button>
      </header>
      ...
      <div className="mode-toggle">
        <label className={mode === 'practice' ? 'selected' : ''}>
          ...
          <span><strong>Oefenen</strong><br />directe feedback en uitleg per vraag</span>
        </label>
        <label className={mode === 'test' ? 'selected' : ''}>
          <input type="radio" name="mode" checked={mode === 'test'} onChange={() => setMode('test')} />
          <span><strong>Test</strong><br />geen tussentijdse feedback, alleen een eindrapport</span>
        </label>
      </div>
